---
title: Accueil
layout: hextra-home
---

<div class="hx:w-full hx:flex hx:flex-col hx:items-center hx:pt-6 hx:pb-4 hx:text-center">

{{< hextra/hero-badge >}}
  👋 Passionné de technologie et d'informatique
{{< /hextra/hero-badge >}}

<div class="hx:mt-6">
{{< hextra/hero-headline >}}
  Dépannages, recherches et documentation technique
{{< /hextra/hero-headline >}}
</div>

<div class="hx:mt-6 hx:max-w-2xl">
{{< hextra/hero-subtitle >}}
  Pannes réellement vécues, explorations techniques, procédures de référence
  et quelques avis tranchés — du réseau au noyau, sans blabla superflu.
{{< /hextra/hero-subtitle >}}
</div>

<div class="hx:mt-8">
{{< hextra/hero-button text="Parcourir la doc" link="docs" >}}
</div>

</div>

<div class="hx:w-full hx:mt-16 hx:mb-6">
<h2 class="hx:not-prose hx:text-2xl hx:font-semibold hx:tracking-tight hx:text-center hx:text-gray-900 hx:dark:text-gray-100">
  Explorer le site
</h2>
</div>

<div class="hx:w-full home-cards">
{{< cards cols="1" >}}
  {{< card link="troubleshooting" title="Troubleshooting" icon="exclamation" subtitle="Pannes rencontrées, diagnostics et résolutions, classées par domaine." >}}
  {{< card link="research" title="Research" icon="beaker" subtitle="Explorations techniques plus poussées sur le noyau, le stockage et la sécurité." >}}
  {{< card link="docs" title="Docs" icon="document-text" subtitle="Documentation de référence, procédures et configurations types." >}}
  {{< card link="decouvertes" title="Découvertes" icon="sparkles" subtitle="Distributions, outils et projets qui valent le détour." >}}
  {{< card link="opinions" title="Opinions" icon="chat-alt" subtitle="Billets et prises de position, au format blog." >}}
{{< /cards >}}
</div>
